...
func SearchHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["auto_correct"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("auto_correct=%v", val))
		}
		...
		if val, ok := args["compatibility_filter"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("compatibility_filter=%v", val))
		}
		if val, ok := args["epid"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("epid=%v", val))
		}
		...
		if val, ok := args["gtin"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("gtin=%v", val))
		}
		...
		url := fmt.Sprintf("%s/item_summary/search%s", cfg.BaseURL, queryString)
...
func CreateSearchTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_item_summary_search",
		...
		mcp.WithString("auto_correct", mcp.Description("A query parameter that enables auto correction. Valid Values: KEYWORD")),
		...
		mcp.WithString("gtin", mcp.Description("This field lets you search by the Global Trade Item Number of the item as defined by https://www.gtin.info. You can search only by UPC (Universal Product Code). If you have other formats of GTIN, you need to search by keyword. For example: /buy/browse/v1/item_summary/search?gtin=099482432621 Maximum: 1 Required: The method must have category_ids, epid, gtin, or q (or any combination of these)")),
